...
description: "notemd had zero file-watching, so external edits were invisible and the editor would silently overwrite them. Three layers later, with a change-gated merge, an editor that reconciles instead of clobbering, and a live FSEvents watcher, Claude and I can edit the same notes."
date: "2026-06-20"
author: Andre
tags: [notemd, sync, fsevents, coredata, swiftui, claude]
---

So here's the thing I actually wanted this week: I store every article in notemd as a plain `.md` file in a vault folder (Obsidian-style, where folders are the hierarchy and filenames are titles). Claude Desktop now has a file connector that can read and write files on disk. Put those two together and you get something I've wanted for ages. Ask Claude to draft, restructure, or fact-check a note, have it write straight into the vault, and watch the change appear in notemd.
...
I also gated the post-merge notification on "did anything actually change", and made it carry the set of `changedArticleIDs` that Layer 3 needs.
...
I went with **FSEvents**, not a `DispatchSource` vnode watch. A vnode source watches a single file descriptor and doesn't recurse into subfolders, which is useless for a folder *tree*. FSEvents recurses, coalesces, and reports cross-process writes, which is exactly the "another app wrote my files" case. The wrapper lives in [`VaultFileWatcher.swift`](notemd/Services/VaultFileWatcher.swift).

The filter is a pure function (the one part worth unit-testing, since FSEvents itself is miserable to test deterministically):
...
While I was in here I noticed two tests that had been red on a clean checkout for a while. Both, it turned out, were *test* bugs rather than product bugs. I fixed them as a separate commit so they wouldn't muddy the sync diff.
...
`mergeProjectVaultDeletesLocalArticleWhenVaultFileIsDeleted` was more interesting, because the "failure" was actually the product doing the right thing. The test had a single note, deleted its only file, and expected the note to vanish. But the deletion safety net deliberately refuses to delete when the vault scan comes back *empty*, because it can't tell "the user deleted everything" from "the external vault is momentarily unreadable", so it keeps your data. The test predated that guard. I rewrote it to use two notes and delete one file, which exercises a genuine single-note deletion without tripping the wipe-protection, and caught a nice gotcha while doing it. Once the merge deletes a managed object, touching its `.id` crashes, so you have to capture the id *before* the merge runs.
