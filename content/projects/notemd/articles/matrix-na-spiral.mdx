...
Here's what I'd built. After the matrix model extracts a cell, the engine validates the supporting quote, checking whether the model's `source_quote` actually appears in one of the retrieved chunks. If yes: confidence `exact`. If close (whitespace-normalized): `supported`. If no: `weak`.
...
Except, and this is the part that took me embarrassingly long to see, **my prompts were telling Gemma the exact opposite**. They told the model that *a missing quote was itself a reason to return N/A*.
...
Read that example again. The excerpt is about a study that *measures* something with a Likert scale, which is a methodology fragment. It isn't even clearly absent of a theoretical framework. It just doesn't mention one in this *one* excerpt. So the example I gave the model for "no usable evidence" was really an example of "uncertain whether the topic is here".
...
That last clause, "not because you're unsure how to phrase the answer", is doing more work than it looks. It names the exact failure mode I'd been creating. Abstention is not for that.
