...
- `parent` was now a derived cache recomputed from the folder layout, so the graph was drawing a hierarchy that no longer meant anything.
...
My first instinct was a radial/sunburst, with the project in the center, folders fanning out as wedges, notes on the rings, wikilinks as chords across the circle. I built the whole thing on SceneKit so that "zoom out" could one day become a real 3D map.
...
2. **The level-of-detail logic hid everything.** I had distance tiers, close and mid and far, that faded notes out as you pulled back. The thresholds were `40`/`120`, but a freshly-fit camera starts at *hundreds* of units, so it opened straight into the "far" tier where note opacity is **zero**. Even unclipped, every node was fully transparent on the first frame.
...
- **Rotation is an arcball, not a turntable.** You grab the point under your cursor and it sticks to your cursor as you drag, so the surface follows your hand. The camera stays put; the globe spins. ([`GraphSceneView.swift`](notemd/Views/Graph/GraphSceneView.swift), if you want the Shoemake math.)
...
- **Hover focus is gentle.** Hovering a note brightens it and its neighbours, shows their titles, and lightly fades the rest, to 60% opacity rather than the near-invisible 12% I started with, because "everything vanished" is not a focus mode.
...
- **It's all monochrome,** matching the rest of notemd. Nodes are shades of grey by kind, folder notes darker and sources lighter, sized by how many links they have.

And a couple of correctness fixes that mattered more than the polish:

- The force simulation is O(n²) per step, so it runs **off the main thread** with a generation token to drop stale results, so opening the graph or toggling sources no longer freezes the UI for a second.
