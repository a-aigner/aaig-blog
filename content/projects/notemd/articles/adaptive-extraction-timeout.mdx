...
The native extractor doesn't stream progress. It runs as one silent batch and prints exactly one thing, the result, at the very end. So "time since last output" is identical to "total time." My inactivity watchdog was a total timeout wearing a disguise. It killed the 56 MB file at the same 5-minute mark. There was never any activity to reset against.
...
Both are knowable. Pages and bytes I can read before extraction even starts (`PDFDocument(url:).pageCount` is basically free). And the machine's speed? I can just *measure it*, because every successful extraction is a data point of "this many pages / this many MB took this many seconds on this Mac."
...
Why both? Because neither one alone is enough. That 794-page book was only 4.6 MB, so it's page-bound. The 56 MB report was the opposite, relatively few pages, but each one a big scanned image, so it's byte-bound. So I predict from each and take the larger:
...
- On a fast Mac, the learned rates are small, so budgets *tighten*, and a hung file is caught in minutes instead of waiting out a padded constant.
- On a slow Mac, the rates are bigger, budgets grow to match, and big files actually finish.

Same code, three machines, three different sets of learned rates. And it's all local. Just samples in `UserDefaults`, nothing phones home. Your M4 and my M1 just quietly disagree about what "a reasonable timeout" means, and they're both right.
...
**A file slower than the budget would fail forever.** Since I only learn from *successful* runs, imagine a file that's genuinely slower than the learned budget: it times out, records nothing, retries with the same budget, times out again… stuck. So on a timeout I record a *censored* sample, meaning "this took at least the budget we gave it." That nudges the rate up, so the retry gets more time, and the budget converges upward until the file finally makes it:
